...
function stabilizeWithMotion(prev, curr) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    out[i] = {
      x: warpedPrev.x * 0.40 + curr[i].x * 0.60,
...
export default function VirtualTryOn() {
  ...
  // LV flow: start with empty right side so divider shows "Add another"
  const [rightShade, setRightShade] = useState(LIPSTICK_SHADES[0]);
  const [activeSide, setActiveSide] = useState("left");

  // Compact compare picker state (LV-style tray)
  const [comparePickerOpen, setComparePickerOpen] = useState(false);

  const leftColorRef = useRef(leftShade.color);
  const rightColorRef = useRef(rightShade.color);
  useEffect(() => { leftColorRef.current = leftShade.color; }, [leftShade]);
  useEffect(() => { rightColorRef.current = rightShade.color; }, [rightShade]);

  const [snapshot, setSnapshot] = useState(null);

  // NEW: shade badge state (shows active side's shade)
  ...
  // Split (dual) state
  const [compareEnabled, setCompareEnabled] = useState(false); // now means Dual Shades
  ...
  useEffect(() => { compareEnabledRef.current = compareEnabled; }, [compareEnabled]);
  useEffect(() => { compareRatioRef.current = compareRatio; }, [compareRatio]);

  const activeShade = activeSide === "left" ? leftShade : rightShade;

  // Keep the active shade centered in the carousel (for single-mode rail)
  useEffect(() => {
    ...
    const target = activeBtn.offsetLeft - scroller.clientWidth / 2 + activeBtn.offsetWidth / 2;
  ...
  // Compare rail scroller (visible only in split mode)
  ...
  // Tighter clamp so labels/handle never escape the canvas edges
  ...
  const toggleCompare = () => {
    setCompareEnabled((prev) => {
      ...
      if (next) {
        ...
        // Open compact tray if right is empty (LV behavior)
  ...
  // === Compare Picker visibility is now inline (always visible in split) ===
  ...
  useEffect(() => {
    ...
    badgeTimerRef.current = setTimeout(() => setShowShadeBadge(false), 1600);
  ...
  function setupCanvas() {
    ...
    if (!tintCanvasLeftRef.current) tintCanvasLeftRef.current = document.createElement("canvas");
    tintCanvasLeftRef.current.width = canvas.width;
    tintCanvasLeftRef.current.height = canvas.height;

    if (!tintCanvasRightRef.current) tintCanvasRightRef.current = document.createElement("canvas");
    ...
    const step = async () => {
      ...
      if (drawLm) {
        ...
        if (ONLY_HIDE_ON_HAND) {
          ...
          HARD_OCCLUSION = handOverlapNow || handOverlapOnStreakRef.current >= HAND_OCCL_ON_FRAMES;
          occlStreakRef.current = 0;
        } else {
        ...
        const anyColorSelected = compareEnabledRef.current
          ? (leftColorRef.current !== "transparent" || rightColorRef.current !== "transparent")
          : (leftColorRef.current !== "transparent");
        const shouldShow = (lipsVisibleNow && !HARD_OCCLUSION) || holdFramesRef.current > 0;
        targetAlphaRef.current = (anyColorSelected && shouldShow) ? 1 : 0;
        ...
        if (willDraw) {
          ...
          if (drawOuter && drawInner) {
            ...
            if (lc !== "transparent") tintOnCtx(tintLeftCtx, lc, drawOuter, drawInner, w, h);
            if (compareEnabledRef.current) {
              // Before user picks a right shade, mirror left to right (LV feel)
              const rightShadeColor = rc !== "transparent" ? rc : lc;
              if (rightShadeColor !== "transparent") tintOnCtx(tintRightCtx, rightShadeColor, drawOuter, drawInner, w, h);
            } else {
              if (lc !== "transparent") tintOnCtx(tintRightCtx, lc, drawOuter, drawInner, w, h);
            }
          }
        }
      ...
      if (compareEnabledRef.current) {
        ...
        frontCtx.save();
        frontCtx.beginPath();
        frontCtx.rect(splitPx, 0, frontCanvas.width - splitPx, frontCanvas.height);
        frontCtx.clip();
  ...
  // Ensure the AR experience fully locks to the viewport without scrollbars
  useEffect(() => {
  ...
  const formatShadeLine = (shade) => {
    ...
    const code = shade.code && String(shade.code).trim().length ? String(shade.code).trim() : null;
      ...
      <div className="relative w-full h-full bg-black">
        ...
        {started && !snapshot && consentAccepted && (
          <div className="absolute top-8 sm:top-10 left-1/2 -translate-x-1/2 text-center z-30 pointer-events-none">
            <div className="text-xs sm:text-sm tracking-[0.3em] uppercase text-white/70">Marvelle Beauté</div>
            <div className="text-base sm:text-lg text-white font-semibold tracking-[0.12em]">Virtual Try-On</div>
          </div>
        )}
          ...
          <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
            <div className="relative w-full max-w-2xl bg-white text-black rounded-[32px] shadow-[0_25px_60px_rgba(0,0,0,0.35)] border border-black/5 p-6 sm:p-10">
              ...
              <div className="space-y-6 sm:space-y-8">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-serif text-black mb-3">Consent Form</h2>
                  <p className="text-sm sm:text-base leading-relaxed text-black/80">
                    To enjoy the Marvella Virtual Try-On (VTO) experience, we need access to your camera. By clicking
                    <span className="font-semibold"> “I want to proceed”</span>, you allow us to process live images of
                    your face to preview lipstick shades in augmented reality. To use this feature you should be at least
                    18 years old. Your video feed stays on your device, is processed in real time for shade rendering,
                    and is never stored or shared with third parties. The imagery will not be reused for unrelated
                    purposes. Review our {" "}
                    <a href="#privacy-policy" className="underline decoration-black/50 underline-offset-4 hover:text-black">
                      Privacy Policy
                    </a>{" "}
        ...
        {consentDeclined && !showConsent && !consentAccepted && (
          <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/75 backdrop-blur-sm px-4">
            <div className="w-full max-w-lg rounded-[28px] border border-white/15 bg-black/85 text-white p-6 sm:p-8 text-center space-y-5 shadow-[0_20px_50px_rgba(0,0,0,0.45)]">
              <h3 className="text-xl sm:text-2xl font-semibold">Consent Required</h3>
              <p className="text-sm sm:text-base text-white/80">
                We need your permission to access the camera before launching the Virtual Try-On experience.
                You can review the consent details again or close this window to exit.
              </p>
        ...
        {/* ===== Shade name + swatch badge overlay (active side) ===== */}
        {started && !snapshot && !compareEnabled && (
          <div
            aria-live="polite"
            className={`pointer-events-none absolute left-1/2 -translate-x-1/2 top-20 sm:top-24 z-20 
                        transition-all duration-300 ease-out 
                        ${showShadeBadge ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2"}`}
          >
        ...
        {/* ===== Split overlay: divider handle + Add Another bubble + Single-shade exit ===== */}
       {/* ===== Split overlay: divider handle + Add Another bubble + Single-shade exit ===== */}
{compareEnabled && started && !snapshot && (
  <>
    {/* Center drag handle only (no dividing line) */}
    <div className="absolute inset-0 z-30 pointer-events-none">
      <div
        className="absolute top-0 bottom-0 w-0"
        style={{ left: `${compareRatio * 100}%`, transform: "translateX(-50%)" }}
      >
        <button
          type="button"
        ...
        >
          ...
          <svg className="w-5 h-5 relative" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    ...
    {/* Bottom panel: refreshed dual-shade tray */}
          ...
          <div className="flex flex-col gap-5">
              ...
              <div className="pointer-events-auto flex items-center gap-4 rounded-2xl border border-white/15 bg-white/5 px-4 py-3 shadow-[0_10px_30px_rgba(0,0,0,0.3)]">
                <span className="inline-flex h-14 w-14 items-center justify-center rounded-full border border-white/30 bg-black/40 shadow-[0_14px_30px_rgba(0,0,0,0.45)]">
                  <span
                    ...
                    style={{ backgroundColor: leftShade.color === "transparent" ? "#6f6f6f" : leftShade.color }}
                    aria-hidden="true"
                  />
                </span>
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] uppercase tracking-[0.4em] text-white/60">Left shade</span>
                  <span className="text-sm sm:text-base font-semibold text-white">{leftShadeLine}</span>
                  <span className="text-[11px] text-white/60">
                    {leftShade.color === "transparent" ? "Live tone" : leftShade.color.toUpperCase()}
                  </span>
                </div>
              </div>

              <div
                className={`pointer-events-auto flex items-center gap-4 rounded-2xl border px-4 py-3 transition-colors shadow-[0_10px_30px_rgba(0,0,0,0.3)] ${rightShade?.id === 0 ? "border-dashed border-white/45 bg-white/5" : "border-white/15 bg-white/5"}`}
                onClick={() => setComparePickerOpen(true)}
              >
                <span className="inline-flex h-14 w-14 items-center justify-center rounded-full border border-white/30 bg-black/40 shadow-[0_14px_30px_rgba(0,0,0,0.45)]">
                  <span
                    className="h-10 w-10 rounded-full border border-white/40"
                    style={{ backgroundColor: rightShade?.color === "transparent" ? "#6f6f6f" : rightShade?.color }}
                    aria-hidden="true"
                  />
                </span>
                <div className="flex flex-1 flex-col gap-1">
                  <span className="text-[10px] uppercase tracking-[0.4em] text-white/60">Right shade</span>
                  <span className={`text-sm sm:text-base font-semibold ${rightShade?.id === 0 ? "text-white/70" : "text-white"}`}>
                    {rightShade?.id === 0 ? "Tap a shade below to add" : formatShadeLine(rightShade)}
                  </span>
                  <div className="flex flex-wrap items-center gap-2 text-[11px] text-white/60">
                    {rightShade?.id === 0 ? (
                      ...
                      <>
                        <span className="font-mono">{rightShade.color?.toUpperCase()}</span>
                        <span className="rounded-full bg-white/15 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.35em]">
                          Active
                        </span>
                      </>
                    )}
                ...
                {rightShade?.id === 0 && (
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); setComparePickerOpen(true); }}
            ...
            <div className="flex flex-col gap-2 px-1">
              ...
              <div className="relative w-full mt-1">
                <button
                  ...
                  className="pointer-events-auto absolute left-0 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 border border-white/20 text-white/80 hover:text-white z-10"
                  aria-label="Scroll shades left"
                >
                  <svg className="mx-auto w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="15 18 9 12 15 6" />
                  </svg>
                </button>

                <div ref={compareScrollerRef} className="hide-scrollbar flex items-center gap-3 overflow-x-auto py-2 px-10">
                  {/* None chip */}
                  <button
                    type="button"
                    onClick={() => { setRightShade(LIPSTICK_SHADES[0]); setComparePickerOpen(true); }}
                    className={`relative flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-full border ${rightShade?.id === 0 ? "border-white/90 ring-2 ring-white/60" : "border-white/25"} text-white/85 pointer-events-auto`}
                  ...
                  {LIPSTICK_SHADES.filter(s => s.id !== 0).map((shade) => (
                    <button
                      key={shade.id}
                      type="button"
                      onClick={() => { setRightShade(shade); setComparePickerOpen(false); }}
                      className={`relative flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-full border ${rightShade?.id === shade.id ? "border-white/90 ring-2 ring-white/60" : "border-white/25"} hover:scale-105 transition pointer-events-auto`}
                ...
                <button
                  type="button"
                  onClick={() => scrollCompareRail(1)}
                  className="pointer-events-auto absolute right-0 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 border border-white/20 text-white/80 hover:text-white z-10"
            ...
            <div className="text-center text-white leading-tight">
              ...
              <div className="mt-1 text-sm sm:text-base font-semibold text-white/90">
                <span className="text-white">Left:</span> {leftShadeLine} <span className="mx-2 opacity-60">|</span> <span className="text-white">Right:</span> {rightShadeLine}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="absolute inset-x-0 bottom-0 pt-5 pb-5 bg-gradient-to-t from-black/75 via-black/25 to-transparent z-10">
            <div className="max-w-6xl mx-auto flex flex-col items-center gap-4 px-4">
              <div className="w-full max-w-4xl flex flex-col items-center gap-2.5">
                {/* Base shade rail (single-mode only) */}
                {!compareEnabled && (
                    ...
                    <div className="flex-1 px-1.5 sm:px-6">
                      ...
                      >
                        {LIPSTICK_SHADES.map((shade) => {
                          const isSelected = (activeSide === 'left' ? leftShade.id : rightShade.id) === shade.id;
                          const shadeColor = shade.color === "transparent" ? "rgba(110,110,110,0.65)" : shade.color;
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              ref={(el) => {
                                if (el) { shadeButtonsRef.current[shade.id] = el; } else { delete shadeButtonsRef.current[shade.id]; }
                              }}
                              onClick={() => { if (activeSide === 'left') setLeftShade(shade); else setRightShade(shade); }}
                              className={`relative flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden transition-transform duration-200 ease-out ${isSelected ? "scale-105" : "hover:scale-105"}`}
                              style={{
                                background: shadeBackground,
                                boxShadow: isSelected
                                  ? "0 0 0 3px rgba(255,255,255,0.95), 0 0 0 6px rgba(0,0,0,0.32)"
                ...
                {/* Compare area (split mode): labels + compact tray */}
                {compareEnabled && (
                  <div className="w-full max-w-5xl">
                    {/* Floating left/right chips (LV style) */}
                    <div className="flex items-center justify-center gap-4 mb-3">
                      <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/60 border border-white/25">
                        <span className="inline-block w-5 h-5 rounded-full border border-white/30" style={{ background: leftShade.color === 'transparent' ? '#6f6f6f' : leftShade.color }} />
                        <span className="text-white text-xs sm:text-sm font-semibold uppercase tracking-[0.22em] whitespace-nowrap">
                          {leftShade.code ? `${leftShade.code} - ${leftShade.name}` : leftShade.name}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/60 border border-white/25">
                        <span className="inline-block w-5 h-5 rounded-full border border-white/30" style={{ background: rightShade.color === 'transparent' ? '#6f6f6f' : rightShade.color }} />
                        <span className="text-white text-xs sm:text-sm font-semibold uppercase tracking-[0.22em] whitespace-nowrap">
                          {rightShade?.id === 0 ? "ADD ANOTHER" : (rightShade.code ? `${rightShade.code} - ${rightShade.name}` : rightShade.name)}
                        </span>
                      </div>
                    </div>

                    {/* Compact compare tray with helper text */}
                    <div className="relative">
                      {(comparePickerOpen || rightShade?.id === 0) && (
                        <div className="mb-1 text-center">
                          <div className="text-white/75 text-[11px] tracking-[0.32em] uppercase">Select a shade to compare</div>
                          <div className="text-white/75 text-[11px]">Tap a shade from the tray</div>
                        </div>
                      )}
                      ...
                      <button
                        type="button"
                        onClick={() => scrollCompareRail(-1)}
                        className="absolute left-0 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/50 border border-white/20 text-white/80 hover:text-white z-10"
                        aria-label="Prev"
                      >
                        <svg className="mx-auto w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
                      </button>

                      <div ref={compareScrollerRef} className="hide-scrollbar flex items-center gap-3 overflow-x-auto py-2 px-9">
                        {/* None chip */}
                        <button
                          type="button"
                          onClick={() => { setRightShade(LIPSTICK_SHADES[0]); setComparePickerOpen(true); }}
                          className={`relative flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-full border ${rightShade?.id === 0 ? 'border-white/90 ring-2 ring-white/60' : 'border-white/25'} text-white/85`}
                          style={{ background: "rgba(110,110,110,0.45)" }}
                          title="None"
                        >
                          <span className="absolute inset-0 flex items-center justify-center">
                        ...
                        {LIPSTICK_SHADES.filter(s => s.id !== 0).map((shade) => (
                          <button
                            ...
                            type="button"
                            onClick={() => { setRightShade(shade); setComparePickerOpen(false); }}
                            className={`relative flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden border ${rightShade?.id === shade.id ? 'border-white/90 ring-2 ring-white/60' : 'border-white/25'} hover:scale-105 transition`}
                            style={{ background: shade.color }}
                            title={shade.name}
                          />
                        ))}
                      ...
                      <button
                        type="button"
                        onClick={() => scrollCompareRail(1)}
                        className="absolute right-0 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/50 border border-white/20 text-white/80 hover:text-white z-10"
                        aria-label="Next"
                      >
                        <svg className="mx-auto w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                      </button>
                    </div>
                  </div>
                )}
              ...
              <div className="w-full max-w-lg flex items-center justify-center gap-6 text-white text-opacity-90">
                <button className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 hover:bg-white/10 transition-colors">
                  <svg className="w-4 h-4 md:w-4.5 md:h-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <button
                  ...
                  className={`flex h-11 w-11 items-center justify-center rounded-full transition-colors border ${compareEnabled ? "bg-white text-black shadow-[0_6px_18px_rgba(0,0,0,0.35)] border-black/20" : "border-white/10 hover:bg-white/10"}`}
                  aria-pressed={compareEnabled}
                  title={compareEnabled ? "Disable dual shades" : "Dual shades split"}
                >
                  <svg className="w-4 h-4 md:w-4.5 md:h-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <button onClick={takeSnapshot} className="relative flex h-[3.25rem] w-[3.25rem] md:h-[3.6rem] md:w-[3.6rem] items-center justify-center rounded-full bg-white shadow-[0_16px_34px_rgba(0,0,0,0.35)] active:scale-95 transition-transform">
                  <div className="h-[85%] w-[85%] rounded-full border-[3px] border-black/80"></div>
                </button>
                <button className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 hover:bg-white/10 transition-colors">
                  <svg className="w-4 h-4 md:w-4.5 md:h-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
